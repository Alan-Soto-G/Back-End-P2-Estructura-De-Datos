...
class Graph:
    ...
    def dfs_all_paths(self, start, end, path=None):
        ...
        if start == end:
            return [list(path)]  # Devolvemos una copia del camino encontrado
        ...
        for neighbor in self.graph[start]:
            if neighbor not in path:  # Evitar ciclos (no visitar un nodo que ya esté en el camino)
                new_paths = self.dfs_all_paths(neighbor, end, path)  # Llamada recursiva
                for new_path in new_paths:
                    paths.append(new_path)  # Guardamos los caminos encontrados
